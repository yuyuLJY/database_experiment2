...
public class Step1 {
	...
	public static class ClusterMapper extends Mapper<Object, Text, Text, Text>{
		public void map(Object key,Text value,Context context) throws IOException, InterruptedException{
			...
			if(size<info.clusterNumber) {//初始阶段，需要选出clusterNumber个点
				System.out.println("初始化添加簇点");
				String newCluster = getRecordString(splitResult);
				System.out.println("簇心"+newCluster);
				info.addClusterList(newCluster);
				//归属关系
				int ClusterNo = info.clusterList.indexOf(newCluster);
				//System.out.println("簇点No:"+ClusterNo);
				context.write(new Text(String.valueOf(ClusterNo)), value);//<"C1",整条信息>
			}else {//
				//开始比较阶段
				String ClusterNo = judgeBelonging(splitResult);
				//int ClusterNo = info.clusterList.indexOf(ClusterNoString);
				//System.out.println("判断簇点No:"+ClusterNo);
				context.write(new Text(ClusterNo), value);//<"C1",整条信息>
			}
		}
	}
